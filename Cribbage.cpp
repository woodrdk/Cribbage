...
void rules() {
    ...
    cout << "From a shuffled pack face down, each player cuts a card, leaving at least four cards at either end of the pack.If both players cut cards of the same rank, each draws again.The player with the lower card deals the first hand.Thereafter, the turn to deal alternates between the two players, except that the loser of the game deals first if another game is played.The dealer has the right to shuffle last, and presents the cards to the non - dealer for the cut prior to the deal. (In some games, there is no cut at this time.)" << endl;
    cout << endl;
    cout << "THE DEAL" << endl;
    ...
    cout << endl;
    cout << "THE CRIB" << endl;
    cout << "Each player looks at their six cards and \"lays away\" two of them face down to reduce the hand to four.The four cards laid away together constitute \"the crib\".The crib belongs to the dealer, but these cards are not exposed or used until after the hands have been played." << endl;
    cout << endl;
    cout << "BEFORE THE PLAY" << endl;
    cout << "After the crib is laid away, the non - dealer cuts the pack.The dealer turns up the top card of the lower packet and places it face up on top of the pack.This card is the \"starter.\" If the starter is a jack, it is called \"His Heels,\" and the dealer pegs(scores) 2 points at once.The starter is not used in the play phase of Cribbage, but is used later for making various card combinations that score points." << endl;
    cout << endl; 
    cout << "THE PLAY" << endl;
    cout << "After the starter is turned, the non - dealer lays one of their cards face up on the table.The dealer similarly exposes a card, then non - dealer again, and so on - the hands are exposed card by card, alternately except for a \"Go,\" as noted below.Each player keeps their cards separate from those of their opponent." << endl;
    cout << "As each person plays, they announce a running total of pips reached by the addition of the last card to all those previously played. (Example: The non - dealer begins with a four, saying \"Four.\" The dealer plays a nine, saying \"Thirteen\".) The kings, queensand jacks count 10 each; every other card counts its pip value(the ace counts one)." << endl;
    cout << endl; 
    cout << "THE GO" << endl;
    cout << "During play, the running total of cards may never be carried beyond 31. If a player cannot add another card without exceeding 31, he or she says \"Go\" and the opponent pegs 1. After gaining the Go, the opponent must first lay down any additional cards he can without exceeding 31. Besides the point for Go, he may then score any additional points that can be made through pairsand runs(described later).If a player reaches exactly 31, he pegs two instead of one for Go." << endl;
    cout << "The player who called Go leads for the next series of plays, with the count starting at zero.The lead may not be combined with any cards previously played to form a scoring combination; the Go has interrupted the sequence." << endl;
    cout << "The person who plays the last card pegs one for Go, plus one extra if the card brings the count to exactly 31. The dealer is sure to peg at least one point in every hand, for he will have a Go on the last card if not earlier." << endl;
    cout << endl; 
    cout << "PEGGING" << endl;
